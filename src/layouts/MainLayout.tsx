...
const handleNavigation = ({ navItem, iteration = 1 }: { navItem: TNavigation, iteration?: number }) => {
    if (navItem.type == 'link' && navItem.path) {
        return (
            <SideNavigation.NavLink key={navItem.key} to={navItem.path} className={`${iteration > 1 ? 'px-14' : ''}`}>
                {navItem.icon}
                {navItem.name}
        ...
        return (
            <SideNavigation.Group key={navItem.key}>
                ...
                <SideNavigation.GroupContent group={navItem.key}>
                    {navItem.submenu.map(navItem => handleNavigation({ navItem, iteration: iteration + 1 }))}
